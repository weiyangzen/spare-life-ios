...
- [x] S3-041 为 `mask update`、`shared stage draft / access / message`、`ritual schedule / complete` 明确其卡片入口、线程入口与错误回退面；当前 `companionContracts.mjs` 已通过 `buildDirectThreadActionEntryModel(...)` / `buildConversationStageContextModel(...).actionLanes[*]` 冻结 `cardEntry / threadEntry / errorFallback`，plugin response 也会把这些 direct-thread lane 追加到 `contextCards`，不再只剩 capability flag。
- [x] S3-042 为 `companion inspect` 定义它在 client 侧的承接位置：诊断入口、内部工具入口或线程附属面，不再悬空；当前 `buildCompanionInspectionEntryModel(...)` 已把它固定为 `diagnostics / internal tool` 承接，明确 `cardEntry/threadEntry = unsupported` 且回退到当前 thread 或 `messages home`，plugin response 会同步透出这份 placement contract。
...
- [x] S3-059 让 `ios/assets` 的资源读取链路在 macOS 宿主可复用，至少覆盖大师目录、图片、基础本地 seed 与诊断页依赖资源；当前 `app/macos` 已补 `HostResources` bundle bridge 与 `Stage3MacOSRuntime.validateHostResources()`，`MasterExperienceStore` 先走 bundled `HostResources`、再回退源码侧 `./ios/assets` / legacy 路径，实际覆盖 `master_service_directory.json`、4 组 `char + image` 大师资源、`seed_manifest.json` 与 `diagnostic_manifest.json`。
...
- [x] S3-071 为 `DesignTokens`、`PlatformCompat`、`WaterfallLayout`、shared feed 组件建立跨 iOS/macOS 的最小兼容检查；当前已在 `PlatformCompat.swift` 冻结 `Stage3SharedSurfaceCompatibilityMatrix.minimumChecks()` 作为显式兼容矩阵，并由 `Docs/Stage3_Shared_Surface_Compatibility.md` 与 `Docs/scripts/validate_stage3_shared_surface.sh` 承接可重复 smoke。
- [x] S3-072 梳理哪些页面当前隐含依赖 UIKit / iOS-only API，列出 macOS UIUX 复刻与 desktop optimization 的 blockers；当前已在 `Docs/Stage3_macOS_UIKit_Blockers.md` 固定 `masters / messages / xianxia / root shell` 的硬 blocker、软 blocker 与推荐抽象路径。
...
- [x] S3-075 对需要 desktop optimization 的页面，优先抽象成共享 view model + 平台专属 layout shell，而不是复制业务逻辑；当前 `UnifiedDiscoverFeedView.swift` 已重构为共享 `DiscoverFeedViewModel` + `SharedFeedLayoutShell` 参考实现，桌面侧只调整外层 workspace shell，不复制 feed 排序与加载逻辑。
