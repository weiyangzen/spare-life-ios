...
PROJECT_PATH = ROOT.join('SpareLifeMastersPreviewHost.xcodeproj')
APP_NAME = 'SpareLifeMastersPreviewHost'
BUNDLE_ID = 'com.wangweiyang.sparelife.masters.previewhost'
DEPLOYMENT_TARGET = '16.0'

HOST_SOURCE_FILES = [
  ROOT.join('App/SpareLifeMastersPreviewHostApp.swift'),
  ROOT.join('App/MastersPreviewRootView.swift')
].freeze
...
def add_source_file(project, target, root_group, file_path, base_root)
...
HOST_SOURCE_FILES.each do |source_file|
  add_source_file(project, target, host_sources_group, source_file, ROOT)
end

SHARED_SOURCE_FILES.each do |source_file|
  add_source_file(project, target, shared_group, source_file, REPO_ROOT)
end
