...
enum MasterCatalogAccessPolicy: String, Equatable {
    ...
    var detail: String {
        switch self {
        case .browseAndChatOnly:
            return "当前只允许浏览目录并进入一对一对话。字段固定来自 ./ios/assets/char，图片固定来自 ./ios/assets/assets；不能在端侧新建、删除或编辑大师资产。"
        }
    }
    ...
    private static func makeProfile(
        ...
        return MasterProfile(
            ...
            assetBundle: MasterAssetBundleInfo(
                ...
                importNote: "目录从 ./ios/assets/char 与 ./ios/assets/assets/char 全量扫描生成，端侧只读消费，并自动补充分组与派生 tags。",
                isOfficial: true,
...
private enum MasterCatalogLoadError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .assetRootsUnavailable:
            return "未找到大师本地资源目录。需要 ./ios/assets/char 与 ./ios/assets/assets/char。"
        case .missingCharacter(let assetID):
            return "缺少大师字段资源：./ios/assets/char/\(assetID).json"
        case .missingImage(let assetID, let fileName):
            return "缺少大师图片资源：./ios/assets/assets/char/\(assetID)/\(fileName)"
