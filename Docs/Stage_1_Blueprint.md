...
其中 `闲人` 与 `闲聊` 在 Stage 1 采用明确的简化版本，不沿用总蓝图的完整能力范围。
...
UI 技术强约束：
Stage 1 默认采用 `SwiftUI` 作为页面实现标准。
如果现有实现中存在旧式 UI 技术或 UIKit 风格承载层，需要按新 UI 标准逐步迁移到 `SwiftUI` 的页面组织、状态管理和交互承接方式，不再继续扩散旧式 UI 实现。
...
Stage 1 的闲人页面只保留 `topic` 与 `topic shards` 两层能力。
...
闲人 Stage 1 的数据与呈现要求，以 [read_clawdb_as_backend_instruction.md](/Users/wangweiyang/GitHub/spare-life-ios/Docs/read_clawdb_as_backend_instruction.md) 为准。

### 闲聊：聚焦目录 + 对话

Stage 1 的闲聊页面只保留两段主链路：
...
当前 Stage 1 先以 `你已经提供的首批大师资源` 作为目录来源，当前这一批恰好是 `8 套`。

约束：

- 当前已经提供的这一批大师资源必须 `全部显示出来`，当前首批就是 8 位，不能漏掉。
- 目录、卡片、进入对话、对话能力全部基于“当前已提供的整批资源”闭环，不允许把当前批次截断成更少条。
- `8` 不是固定上限，只是当前首批数量。
- 后续当资源扩展到几十位或上百位时，目录必须支持 `按批懒加载 / 分页继续加载`，不能把 8 位写死成结构上限。
...
- 闲人：首页与 topic feed [XianxiaHomeView.swift](/Users/wangweiyang/GitHub/spare-life-ios/spare-life-ios-app/Features/Xianxia/XianxiaHomeView.swift)
- 闲人：topic 详情与 shards 承接 [SceneTopicView.swift](/Users/wangweiyang/GitHub/spare-life-ios/spare-life-ios-app/Features/Xianxia/SceneTopicView.swift)

- 闲聊：首页 [MasterHomeView.swift](/Users/wangweiyang/GitHub/spare-life-ios/spare-life-ios-app/Features/Masters/MasterHomeView.swift)
- 闲聊：目录与对话状态承接 [MasterExperienceStore.swift](/Users/wangweiyang/GitHub/spare-life-ios/spare-life-ios-app/Features/Masters/MasterExperienceStore.swift)
